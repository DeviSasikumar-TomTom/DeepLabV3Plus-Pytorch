...
def get_argparser():
    ...
    parser.add_argument("--label_map", type=str, default='./datasets/data/label_mapping_config.yaml',
                        help="path to label mapping config file")
        ...
        for step in range(3):
            ...
            if (cur_itrs) % opts.val_interval == 0:
                ...
                if vis is not None:  # visualize validation score and samples
                    ...
                    # changed code here down:
                    # for k, (img, target, lbl) in enumerate(ret_samples):
                    ret_samples_enu = enumerate(ret_samples)
                    for step in range(2):
                        batch1 = next(ret_samples_enu)
                        k, sample = batch1
                        img = sample["image"]
