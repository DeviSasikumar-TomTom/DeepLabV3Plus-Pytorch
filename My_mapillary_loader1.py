    ...
    def transforms_tr(self, sample):
        ...
        if prob_augmentation > 0.1:
            ...
            if sample['image'].shape[0] < 360 or sample['image'].shape[1] < 480:
                 ...
                 composed_transforms = tr.Compose([tr.Resize((360, 480)),
                                                          tr.RandAugment(3, augmentation_strength),
                                                          tr.LabelMapping(self.label_mapping), tr.ToTensor()])

            else:
                composed_transforms = tr.Compose([tr.RandomCrop((360, 480)),
                                                          tr.RandAugment(3, augmentation_strength),
...
if __name__ == "__My_mapillary_loader1__":
    ...
    parser.add_argument("--data-dir", type = str, default='./data/Mapillary/training',  help = "Path to the directory containing dataset")
    parser.add_argument("--label-mapping-config", type = str, default='./label_mapping_config.yaml', help = "path to label mapping config yaml")
